...
function ssss(lat, lon){
    ...
    fetch("/api/data/store-key")
        .then(res => res.text())
        .then(apiKey => {
            let url = `${param.serverUrl}?serviceKey=B+0kREGdToh/cNzAmpRwP/FpKU6em6Y+MFNKi7Z4D1oBS8Nqf9qRKWF5RPk8RRfx9Nv0mhuYMkWDjXChTwCDGA==&pageNo=${param.pageNo}&cx=${lon}&cy=${lat}&numOfRows=${param.numOfRows}&type=${param.type}`;
            console.log(apiKey);
            console.log(url);
            try {
                fetch(url)
