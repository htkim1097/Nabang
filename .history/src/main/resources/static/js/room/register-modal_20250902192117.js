...
document.getElementById("submit-btn").addEventListener("click", async e => {
    try {
        ...
        // 상권 근접 정도
        plainFormData.storeScore = getStoreConunt(lat, lon, 1000);

        // 소음 정도
        plainFormData.noiseSaftyScore = getStoreConunt(lat, lon, 10);
...
async function getStoreConunt(lat, lon, radius){
    ...
    await fetch("/api/data/store-key")
        .then(res => res.text())
        .then(apiKey => {
            ...
            try {
                fetch(url)
                    .then(res => res.json())
                    .then(data => {
                        const storeCnt = data.body.totalCount;
